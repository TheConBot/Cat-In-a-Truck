...
public class CookingContainer : Container {
  ...
  public void AddToContainer(SolidIngredient newIngredient) {
    ...
    if(index < 0) {
      Debug.LogError("Your check to see if the container was full didnt work for some reason bud.");
      return;
    }
      ingredientsToHold[index] = newIngredient;
  ...
  private IEnumerator CookFood(SolidIngredient ingredient) {
    ...
    Debug.Log("You ruined the " + ingredient.DisplayName);
    yield return null;
  }
}
